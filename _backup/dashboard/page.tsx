...
export default function DashboardClientLayout({
  ...
  return (
    <div className="relative min-h-screen flex flex-col md:flex-row overflow-hidden bg-gray-100">
      {/* Modern sidebar - responsive width and layout */}
      <nav className="w-full md:w-36 bg-white shadow-sm z-10 flex flex-row md:flex-col border-b md:border-b-0 md:border-r border-gray-200">
        <div className="p-4 md:p-5 border-r md:border-r-0 md:border-b border-gray-200 flex-1 md:flex-none">
          <h1 className="text-lg md:text-xl font-bold text-gray-800 tracking-tight">
            Account Management.
          </h1>
          <p className="text-xs text-gray-500 mt-1 hidden md:block"></p>
        </div>

        <ul className="flex md:flex-1 py-2 md:py-5 space-x-2 md:space-x-0 md:space-y-1 overflow-x-auto md:overflow-visible justify-center md:justify-start">
          <li>
            <button
              onClick={handleStudentClick}
              className="flex items-center w-full px-3 md:px-5 py-2 text-gray-700 hover:text-indigo-500 hover:bg-indigo-400 transition-all duration-200 rounded-lg mx-1 md:mx-3 group text-xs md:text-sm"
            >
      ...
      {/* Main content */}
      <main className="flex-1 p-2 md:p-3 z-10 overflow-auto">
        <div className="bg-gray-100 rounded-xl shadow-sm border border-gray-300 p-3 md:p-4">
          {showBranchSelection ? (
              ...
              {!selectedBranch ? (
                <>
                  ...
                  <input
                    ...
                    className="w-full p-2 text-gray-800 text-sm border border-gray-200 rounded-lg focus:ring-2 focus:ring-indigo-200 focus:border-indigo-400 outline-none transition"
                  />

                  <div className="mb-3">
                    <label className="block text-gray-700 font-medium text-xs md:text-sm mb-1">
                  ...
                  <div className="bg-gray-400 rounded-lg border border-gray-300 p-3 md:p-4 shadow-sm">
                    <div className="flex justify-between items-center mb-3 md:mb-4">
                      <button onClick={goToPreviousMonth} className="px-2 md:px-3 py-1 bg-gray-100 hover:bg-gray-100 rounded-lg text-gray-600 text-xs md:text-sm transition">◀</button>
                      ...
                      <button onClick={goToNextMonth} className="px-2 md:px-3 py-1 bg-gray-50 hover:bg-gray-100 rounded-lg text-gray-600 text-xs md:text-sm transition">▶</button>
                    </div>

                    <div className="grid grid-cols-7 gap-0.5 md:gap-1 text-center mb-2">
                      {weekdays.map((day) => (
                        <div key={day} className="font-medium text-gray-500 text-[10px] md:text-xs">{day}</div>
                      ))}
                    </div>

                    <div className="grid grid-cols-7 gap-0.5 md:gap-1">
                      {calendarDays.map((day, index) => {
                        if (day === null) return <div key={`empty-${index}`} className="p-1 md:p-2" />;
                        ...
                        return (
                          <button
                            ...
                            className={`p-1 md:p-2 text-center text-[11px] md:text-sm rounded-full transition ${
                    ...
                    <div className="mt-3 md:mt-4 flex justify-end">
                      <button onClick={clearSelectedDate} className="px-2 md:px-3 py-1 bg-red-500 hover:bg-red-600 rounded-lg text-white text-xs transition">
                        Clear
                      </button>
                    </div>
                  </div>
                  ...
                  {/* OVERALL PERFORMANCE */}
                  <div className="p-3 md:p-4 bg-gray-50 rounded-lg border border-gray-300">
                    <p className="text-xs md:text-sm text-gray-700">Total Students: <span className="font-semibold">{loadingTotal ? "Loading..." : totalAllStudents}</span></p>
                    <p className="text-xs md:text-sm text-gray-700 mt-1">Total Branches: <span className="font-semibold">{(firebaseBranches.length || branches.length)}</span></p>
                    <p className="text-xs md:text-sm text-gray-700 mt-1">Selected Date: <span className="font-semibold">{selectedDate || "None"}</span></p>
                  </div>
                  ...
                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 md:gap-4 max-h-[400px] overflow-y-auto">
                    {filteredBranches.map((branch) => (
                      <button
                        ...
                        className={`bg-white hover:bg-indigo-200 transition-all p-3 md:p-4 rounded-xl text-left border ${
                      ...
                      >
                        ...
                        <div className="text-gray-500 text-[11px] md:text-xs mt-1">
                  ...
                  {selectedBranch && studentCount !== null && (
                    <div className="mt-4 md:mt-5 p-3 md:p-4 bg-indigo-50 rounded-xl border border-indigo-100">
                      ...
                      <button
                        ...
                        className="mt-2 px-2 md:px-3 py-1 bg-white hover:bg-gray-50 rounded-lg text-gray-600 text-xs transition border border-gray-200"
                      >
                        Clear selection
                      </button>
                    </div>
                  ...
                  <button
                    ...
                    className="mt-2 md:mt-3 px-3 md:px-4 py-1.5 md:py-2 bg-orange-400 hover:bg-gray-100 rounded-lg text-gray-600 text-xs md:text-sm transition font-medium border border-gray-200"
                  >
                    ← Back to dashboard
                  </button>
                </>
                ...
                <div className="space-y-3 md:space-y-4">
                  <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-2">
                    ...
                    <button
                      ...
                      className="px-2 md:px-3 py-1 bg-orange-400 hover:bg-gray-100 rounded-lg text-gray-600 text-xs md:text-sm border border-gray-200"
                    >
                      ← Back to Branches
                    </button>
                  </div>
                  ...
                  <div className="bg-white p-3 md:p-4 rounded-lg border border-gray-200">
                    <h3 className="text-sm md:text-md font-semibold text-gray-800 mb-2 md:mb-3">Filter Collections by Registration Date</h3>
                    <div className="flex flex-wrap gap-2 md:gap-4 items-end">
                      <div className="flex-1 min-w-[120px]">
                        <label className="block text-gray-600 text-xs md:text-sm mb-1">From Date</label>
                        <input
                          type="date"
                          value={startDate}
                          onChange={(e) => setStartDate(e.target.value)}
                          className="border border-gray-300 p-1.5 md:p-2 rounded-lg bg-white text-gray-800 text-xs md:text-sm focus:ring-2 focus:ring-indigo-200 focus:border-indigo-400 outline-none w-full"
                        />
                      </div>
                      <div className="flex-1 min-w-[120px]">
                        <label className="block text-gray-600 text-xs md:text-sm mb-1">To Date</label>
                        <input
                          type="date"
                          value={endDate}
                      ...
                      <button
                        ...
                        className="px-2 md:px-3 py-1.5 md:py-2 bg-red-500 hover:bg-red-600 text-white rounded-lg text-xs md:text-sm transition"
                      >
                        Clear Dates
                      </button>
                    </div>
                    <div className="mt-3 md:mt-4 grid grid-cols-1 sm:grid-cols-2 gap-3 md:gap-4">
                      <div className="bg-green-50 rounded-lg p-2 md:p-3 border border-green-200">
                        <p className="text-green-700 text-xs md:text-sm">Total Collected (Ksh)</p>
                        <p className="text-xl md:text-2xl font-bold text-green-800">{totalCollected.toLocaleString()}</p>
                      </div>
                      <div className="bg-orange-50 rounded-lg p-2 md:p-3 border border-orange-200">
                        <p className="text-orange-700 text-xs md:text-sm">Total Outstanding Debt (Ksh)</p>
                        <p className="text-xl md:text-2xl font-bold text-orange-800">{totalDebt.toLocaleString()}</p>
                      </div>
                    </div>
                  ...
                  {loadingStudents && <div className="text-center py-4 md:py-6 text-gray-500 text-xs md:text-sm">Loading students...</div>}
                  {studentsError && <div className="text-red-600 p-2 md:p-3 bg-red-50 rounded text-xs md:text-sm">{studentsError}</div>}
                  {!loadingStudents && branchStudents.length === 0 && !studentsError && (
                    <div className="p-3 md:p-4 bg-gray-50 rounded-lg border border-gray-200 text-center text-gray-500 text-xs md:text-sm">
                  ...
                  {!loadingStudents && branchStudents.length > 0 && (
                    <div className="space-y-2 max-h-[600px] overflow-y-auto">
                      {branchStudents.map((student) => (
                        <div key={student.id} className="bg-white rounded-lg border border-gray-200 p-3 md:p-4 shadow-sm hover:shadow transition">
                          <div className="grid grid-cols-1 sm:grid-cols-2 gap-1.5 md:gap-2 text-xs md:text-sm">
                            <div><span className="font-medium text-gray-700">Admission No:</span> <span className="text-gray-800 break-all">{student.admissionNumber}</span></div>
            ...
            // ✅ Responsive history section: stack on mobile, side‑by‑side on desktop
